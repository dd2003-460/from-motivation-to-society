...
\begin{document}
	...
	你有没有想过，为什么Evolution青睐某些物种，而淘汰其他物种？自然选择不是"设计者"，而是一个过滤器：它不创造最优解，只是把不符合环境的变体一一筛除。在这个意义上，"适者生存"的真正含义是"不适者消亡"——剩下的，不过是侥幸活下来的样本。
...
\subsection{逻辑衔接}
从晶体到生物组织的演化展示了结构如何在物理化学层面自发涌现并复杂化。但并非所有结构都能长久存续，也不是所有配置都能被保留。下一节将引入自然选择作为过滤器：在竞争与环境的压力下，哪些结构能够幸存，哪些会被淘汰，取决于它们对效率的优化程度。
